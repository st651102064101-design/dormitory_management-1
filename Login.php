...
// ตรวจสอบ Google error
if (isset($_GET['google_error'])) {
    $login_error = $_GET['google_error'];
}
...
try {
    $pdo = connectDB();
    $settingsStmt = $pdo->query("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('site_name', 'logo_filename', 'theme_color', 'public_theme', 'google_client_id', 'line_login_channel_id')");
    while ($row = $settingsStmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($row['setting_key'] === 'line_login_channel_id') $lineLoginChannelId = $row['setting_value'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $redirectUrl = '/dormitory_management/Reports/dashboard.php';
  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      .google-btn:active, .line-btn:active {
        transform: translateY(0);
      }

      .google-btn svg, .line-btn svg {
        width: 20px;
        height: 20px;
        flex-shrink: 0;
      }

      .google-btn span, .line-btn span {
        white-space: nowrap;
      }
      ...
      /* Google & LINE Button for Dark Theme */
      body.theme-dark .google-btn, body.theme-dark .line-btn {
        background: #1a1a1a;
        border: 1px solid #2a2a2a;
        color: #ffffff;
      }

      body.theme-dark .google-btn:hover {
        background: #2a2a2a;
        border-color: #3a3a3a;
        box-shadow: none;
      }
      
      body.theme-dark .line-btn:hover {
        background: #2a2a2a;
        border-color: #06c755;
        box-shadow: none;
      }
...
<?php if (!empty($googleClientId) || !empty($lineLoginChannelId)): ?>
          <div class="login-divider">
            <span>หรือ</span>
          </div>
          <?php endif; ?>

          <?php if (!empty($lineLoginChannelId)): ?>
          <!-- LINE Login (สำหรับผู้เช่า) -->
          <a href="line_login.php" class="line-btn">
            <svg viewBox="0 0 24 24" fill="#06C755" xmlns="http://www.w3.org/2000/svg">
              <path d="M22.5 10.4c0-4.3-4.7-7.8-10.5-7.8S1.5 6.1 1.5 10.4c0 3.8 3.7 7 8.5 7.7.4.1.9.3 1 .6l.3 1.8c0 .2.2.3.4.2 1.6-1.1 5.9-4 8.2-6.2 1.5-1.3 2.6-2.7 2.6-4.1zM9.5 12.3c0 .3-.2.5-.5.5H6.2c-.3 0-.5-.2-.5-.5V8.5c0-.3.2-.5.5-.5h2.8c.3 0 .5.2.5.5s-.2.5-.5.5H7.2v1h1.8c.3 0 .5.2.5.5s-.2.5-.5.5H7.2v1h1.8c.3 0 .5.2.5.5zM13.6 12.3c0 .3-.2.5-.5.5h-1c-.3 0-.5-.2-.5-.5V8.5c0-.3.2-.5.5-.5h1c.3 0 .5.2.5.5v3.8zM17.4 12.3c0 .3-.2.5-.5.5h-2.1c-.3 0-.5-.2-.5-.5V8.5c0-.3.2-.5.5-.5h.6c.3 0 .5.2.5.5v2.8h1.5c.3 0 .5.2.5.5z"/>
            </svg>
          ...
          <p class="google-login-note">
            ...
            <?php if (!empty($googleClientId) && !empty($lineLoginChannelId)): ?>
              Google สำหรับผู้เช่า/เจ้าของ, LINE สำหรับผู้เช่า
            <?php elseif (!empty($lineLoginChannelId)): ?>
              สำหรับผู้เช่า
            <?php else: ?>
