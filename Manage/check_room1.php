...
// Check workflow step
$wfStmt = $pdo->query("SELECT current_step FROM tenant_workflow WHERE ctr_id = {$contract['ctr_id']}");
$workflow = $wfStmt->fetch(PDO::FETCH_ASSOC);
$step = $workflow ? $workflow['current_step'] : 0;
echo "📊 Workflow: Step $step/5\n\n";

// Check checkin
$checkinStmt = $pdo->query("SELECT COUNT(*) as cnt FROM checkin_record WHERE ctr_id = {$contract['ctr_id']}");
$checkin = $checkinStmt->fetch(PDO::FETCH_ASSOC);
echo "✅ Checkin Records: " . $checkin['cnt'] . "\n\n";

// Check utility
$utilStmt = $pdo->query("SELECT COUNT(*) as cnt FROM utility WHERE ctr_id = {$contract['ctr_id']}");
$util = $utilStmt->fetch(PDO::FETCH_ASSOC);
echo "📊 Utility Records: " . $util['cnt'] . "\n";

if ($util['cnt'] > 0) {
    $utlList = $pdo->query("SELECT utl_id, utl_date FROM utility WHERE ctr_id = {$contract['ctr_id']} ORDER BY utl_date DESC");
