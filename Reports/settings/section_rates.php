  ...
  <div class="apple-section-card">
    ...
    if (typeof window.openRatesSheetFromRow !== 'function') {
      window.openRatesSheetFromRow = function(event, rowId) {
        ...
        if (!overlay) {
            // Try to find it by class or exact strict title if ID was stripped
            var allOverlays = Array.from(document.querySelectorAll('.apple-sheet-overlay'));
            overlay = allOverlays.find(o => {
                var titleEl = o.querySelector('.apple-sheet-title');
                if (titleEl && titleEl.textContent && titleEl.textContent.trim() === 'อัตราค่าน้ำค่าไฟ') {
                    return true;
                }
                return false;
            }) || null;
...
<style>
/* Override any potential CSS blockers or caching holding back the visual display of the active sheet */
#sheet-rates.active {
...
#sheet-rates.active .apple-sheet {
