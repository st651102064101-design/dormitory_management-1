    ...
    $sql = "
        ...
        LEFT JOIN (
            SELECT dr.ctr_id,
                   ...
                   MAX(CASE WHEN dr.refund_status = '0' THEN 1 ELSE 0 END) AS refund_pending
                                ...
                                <tr data-wiz-group="<?php echo $isRowCompleted ? 1 : 0; ?>"<?php if ($isCancelPending || $isBookingCancelled): ?> style="background:rgba(239,68,68,0.05)!important;border-left:3px solid rgba(239,68,68,0.45);"<?php endif; ?>>
                                    ...
                                    <td data-label="สถานะ">
                                        <div class="step-indicator">
                                            ...
                                            <div class="step-circle <?php echo $step2 ? 'completed' : ($currentStep == 2 ? 'current' : 'pending'); ?>" data-tooltip="2. ยืนยันชำระเงินจอง" <?php if ($step2 || $currentStep == 2): ?>onclick="openPaymentModal(<?php echo (int)$tenant['bp_id']; ?>, <?php echo (int)$tenant['bkg_id']; ?>, <?php echo htmlspecialchars(json_encode($tenant['tnt_id']), ENT_QUOTES, 'UTF-8'); ?>, <?php echo htmlspecialchars(json_encode($tenant['tnt_name']), ENT_QUOTES, 'UTF-8'); ?>, <?php echo htmlspecialchars(json_encode($tenant['room_number']), ENT_QUOTES, 'UTF-8'); ?>, <?php echo (int)($tenant['bp_amount'] ?? 0); ?>, <?php echo htmlspecialchars(json_encode($tenant['bp_proof'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>, <?php echo $step2 ? 'true' : 'false'; ?>)" style="cursor: pointer;"<?php endif; ?>>
                                    ...
                                    <td data-label="ขั้นตอนถัดไป">
                                        ...
                                        ?>
                                            <div style="display:flex;flex-direction:column;align-items:flex-start;gap:0.4rem;">
                                                ...
                                                <div style="display:flex;flex-wrap:wrap;gap:0.4rem;align-items:center;">
                                                    ...
                                                    <?php elseif ($needsRefund && !$refundDone && (((string)($tenant['ctr_status'] ?? '') === '1') || ((string)($tenant['ctr_status'] ?? '') === '2'))): ?>
                                                    <button type="button"
                                        ...
                                        <?php elseif ($currentStep == 3): ?>
                                            <?php $isContractCreated = (int)($tenant['step_3_confirmed'] ?? 0) === 1; ?>
                                            <?php if ($isContractCreated): ?>
                                        ...
                                        <?php elseif ($currentStep == 4): ?>
                                            <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
                                                ...
                                                <?php if ($isCancelPending): ?>
                                                    <div style="display:inline-flex;align-items:center;gap:0.35rem;padding:0.25rem 0.65rem;border-radius:16px;border:1px solid rgba(239,68,68,0.4);background:rgba(239,68,68,0.1);color:#f87171;font-size:0.78rem;font-weight:700;">แจ้งยกเลิก</div>
                                                <?php endif; ?>
                                                <button type="button" class="action-btn btn-danger" onclick="cancelBooking(<?php echo (int)$tenant['bkg_id']; ?>, <?php echo htmlspecialchars(json_encode($tenant['tnt_id']), ENT_QUOTES, 'UTF-8'); ?>, <?php echo htmlspecialchars(json_encode($tenant['tnt_name']), ENT_QUOTES, 'UTF-8'); ?>)">ยกเลิก</button>
                                            </div>
                    ...
                    <div style="margin-bottom: 1.5rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            ...
                            <div>
                                ...
                                <input type="text" name="water_meter_start" id="checkin_water_meter" maxlength="7" placeholder="เลขมิเตอร์" inputmode="numeric" pattern="\d*"
                            ...
                            <div>
                                ...
                                <input type="text" name="elec_meter_start" id="checkin_elec_meter" maxlength="5" placeholder="เลขมิเตอร์" inputmode="numeric" pattern="\d*"
                                    style="width: 100%; padding: 0.875rem 0.75rem; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 10px; color: #f1f5f9; font-size: 1rem;"
...
<script>
        ...
        const paymentRows = payments.length
            ? payments.map((pay) => {
                ...
                const statusBadge = payStatus === '1'
                    ...
                    : payStatus === '2'
                        ? `<span style="display:inline-flex;align-items:center;gap:0.35rem;padding:0.3rem 0.7rem;border-radius:8px;background:linear-gradient(135deg,rgba(239,68,68,0.2),rgba(220,38,38,0.15));color:#dc2626;font-size:0.8rem;font-weight:700;border:1.5px solid rgba(239,68,68,0.5);box-shadow:0 2px 8px rgba(239,68,68,0.2);"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width:13px;height:13px;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg><strong>ตีกลับแล้ว</strong></span>`
                        : canReview
    ...
    (function() {
        ...
        modalEl.style.cssText = 'display:none;position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.25);backdrop-filter:blur(15px);-webkit-backdrop-filter:blur(15px);align-items:center;justify-content:center;overflow:auto;padding:1rem;';
        modalEl.innerHTML = `
    ...
    function _getRefundStageKey(ctrId) {
        return '_refundStage_ctr_' + (Number(ctrId) || 0);
    }

    function _loadRefundStage(ctrId) {
        const key = _getRefundStageKey(ctrId);
        let stage = null;
        try { stage = sessionStorage.getItem(key); } catch (e) { stage = null; }
        if (!stage) {
            try { stage = localStorage.getItem(key); } catch (e) { stage = null; }
        }
        return stage || null;
    }

    function _saveRefundStage(ctrId, stage) {
        const key = _getRefundStageKey(ctrId);
        try { sessionStorage.setItem(key, stage); } catch (e) {}
    ...
    function openRefundModal(ctrId, tntName, roomNumber, bankName, bankAccName, bankAccNum, depositAmt, refundPending) {
        ...
        const savedRefundStage = _rfCtrId ? _loadRefundStage(_rfCtrId) : null;
        ...
        const shouldShowConfirm = savedRefundStage === 'confirm' || (savedRefundStage === null && ((_rfSavedRefundCtrId && _rfSavedRefundCtrId === _rfCtrId) || refundPending));
    ...
    function closeRefundModal() {
        ...
        if (confirmArea && confirmArea.style.display !== 'none') {
            _saveRefundStage(_rfCtrId, 'confirm');
        } else {
            _saveRefundStage(_rfCtrId, 'edit');
        }
