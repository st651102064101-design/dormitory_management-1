    ...
    try {
      ...
      if (result.success) {
        ...
        if (googleLinkWrap) {
          ...
          // ✅ Re-attach event listener ให้ปุ่ม link ที่สร้างใหม่
          const newLinkBtn = googleLinkWrap.querySelector('.google-link-btn');
          if (newLinkBtn) {
            newLinkBtn.addEventListener('click', async (e) => {
              ...
              const checkClosedInterval = setInterval(async () => {
                if (popup.closed) {
                  ...
                  try {
                    const linkedResponse = await fetch('/dormitory_management/api/check_google_link.php', {
                      method: 'GET',
                    ...
                    if (linkedResult.success && linkedResult.linked) {
